...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
   ...
   * partition to avoid recomputation of UDFs, just like [[CacheProject]].
   *
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    /* Use the CS186Utils functions to generate a function that takes an input iterator and returns an iterator
       that is the result of a caching projection. */
    val generator: (Iterator[Row] => Iterator[Row]) = CS186Utils.generateCachingIterator(projectList, child.output)

    // Hash the relation.
    val hashedRelation = DiskHashedRelation(input, keyGenerator)
    val partitionIterator = hashedRelation.getIterator()

    // Create an iterator that reads one partition in at a time, constructs a CacheProject operator for it,
    // and exhausts that iterator.
    new Iterator[Row] {
      var currentIterator: Iterator[Row] = null

      def hasNext() = {
        val result = (currentIterator != null && currentIterator.hasNext) || (partitionIterator.hasNext && fetchNextPartition())

        // If we have no more rows to read, close all the partitions.
        if(!result) {
      ...
      private def fetchNextPartition(): Boolean  = {
        // Get the next partition and its data.
        val partition = partitionIterator.next()
        val partitionData: Iterator[Row] = partition.getData()

        // Use the generator to get the result iterator for this partition.
        currentIterator = generator(partitionData)
