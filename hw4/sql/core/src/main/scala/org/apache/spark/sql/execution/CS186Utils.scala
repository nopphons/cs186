...
object CS186Utils {
  ...
  def getUdfFromExpressions(expressions: Seq[Expression]): ScalaUdf = {
    ...
    // Find which one of the operators is a UDF.
    for (expression <- expressions) {
      udf = expression match {
        case e: ScalaUdf =>  e
        case _ => null
      }
    }
  ...
  def generateCachingIterator(
    ...
    udf match {
      // If there is no UDF, then do a regular projection operation.
      case null => {
        { input =>
    ...
    { input =>
      new Iterator[Row] {
        ...
        def next() = {
          ...
          val nextKey = cacheKeyProjection.apply(nextValue)
          // For each key, check if it is in the cache; if it is, return it. Else, compute the UDF.
          val udfResult: Row = cache.get(nextKey) match {
            case row: Row => row
            case null => {
              val result = udfProject.apply(nextValue)
              cache.put(nextKey, result)
              result
            }
          }

          new GenericRow((preUdfProjection.apply(nextValue) ++ udfResult ++ postUdfProjection.apply(nextValue)).toArray)
        }
      }
    }
  }
