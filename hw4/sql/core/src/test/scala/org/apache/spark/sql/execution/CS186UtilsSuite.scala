...
class CS186UtilsSuite extends FunSuite {
  ...
  test("sequence with no UDFs") {
    val udf: ScalaUdf = CS186Utils.getUdfFromExpressions(studentAttributes)
  ...
  test("sequence with multiple UDF") {
    val udf1: ScalaUdf = new ScalaUdf((i: Int) => i + 1, IntegerType, Seq(studentAttributes(0)))
    val udf2: ScalaUdf = new ScalaUdf((i: Int) => i - 1, IntegerType, Seq(studentAttributes(0)))
    val attributes: Seq[Expression] = Seq(udf2) ++ Seq() ++ studentAttributes ++ Seq(udf1)

    assert(CS186Utils.getUdfFromExpressions(attributes) == udf1)
  }
}
